...
export type ChatWebviewToExtensionCommand = 
    | { command: 'sendMessage'; text: string; editOptions?: { mode: 'truncate' | 'fork'; index: number }; framingId?: string }
...
export type ChatExtensionToWebviewCommand = 
    | { command: 'initState'; modelName: string; messages: ChatMessage[]; total: number; activeFramingId?: string; activeFramingName?: string }
    | { command: 'setMessages'; messages: ChatMessage[] }
    | { command: 'updateFraming'; framingId?: string; framingName?: string }
    | { command: 'addMessage'; role: 'user' | 'assistant' | 'system' | 'error'; content: string; systemTurnPrefix?: string; userPrefix?: string; userSuffix?: string; systemTurnSuffix?: string; framingId?: string; framingName?: string; modelName?: string; isError?: boolean }
    | { command: 'startAssistantMessage'; modelName?: string }
    | { command: 'appendToken'; content: string }
    | { command: 'endAssistantMessage' }
    | { command: 'enterEditMode'; mode: 'truncate' | 'fork'; index: number; content: string; framingId?: string; framingName?: string }
