...
suite('ChatOrchestrator Integration', function () {
    ...
    class MockChatRepo implements IChatRepository {
        ...
        getAll() { return Array.from(this.chats.values()); }
        getById(id: string) { return this.chats.get(id); }
        async save(chats: any[]) { chats.forEach(c => this.chats.set(c.id, c)); }
        async saveOne(chat: any) { this.chats.set(chat.id, chat); }
    }
    ...
    class MockSettingsRepo implements IModelSettingsRepository {
        ...
        async save(settings: Record<string, ModelInstance>) { this.settings = settings; }
    }
