...
- [x] Task: Update Model Setup Interface b0bc595
    - [x] Modify `src/panels/setupPanel.ts` to integrate the "Apply Template" action.
    - [x] Implement a command to show a template selection QuickPick.
- [x] Task: Implement Template Application Logic b0bc595
    - [x] Overwrite the model's system prompt and trigger a refresh of the setup screen.
- [x] Task: Write Tests for Integration b0bc595
    - [x] Verify that applying a template correctly updates model settings without side effects.
...
- [x] Task: Implement Safety Confirmation Logic b0bc595
    - [x] Implement the template count calculation for tag deletion warnings.
- [x] Task: Write Tests for Safety Logic b0bc595
    - [x] Verify confirmation flow and accurate template count reporting.
- [x] Task: Conductor - User Manual Verification 'Phase 5' (Protocol in workflow.md) b0bc595
