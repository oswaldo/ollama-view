...
*   **Backward Compatibility & Data Preservation**: This is essential. New features must maintain compatibility with existing workflows and data structures. Data MUST NOT be lost when moving between extension versions (forward/backward) or during manual user edits. Unknown fields in JSON must be preserved, and atomic write operations should be used to prevent corruption. All structural changes must be verified with unit tests.
